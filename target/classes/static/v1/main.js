document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
        ...
        // Najpierw sprawdź profil czy nie ma PAUZY
            ...
            .then(db => {
                ...
                if (db.isPaused) {
                    if (!document.getElementById('pause-overlay')) {
                        ...
                        // Kiedy pauza – przesuwamy startMs do przodu o 10s żeby nie naliczać czasu
                        if (levelStartMs) levelStartMs += 10000;
                    } else {
                        // Jeśli już jest, to tylko przesuwamy startMs
                        if (levelStartMs) levelStartMs += 10000;
                    }
                    return; // Nie wysyłaj postępu gdy pauza
                } else {
                    const ov = document.getElementById('pause-overlay');
                    if (ov) ov.remove();
                }

                if (levelStartMs && currentLevel > 0) {
                    let currentElapsed = Date.now() - levelStartMs;
                    ...
                    fetch('/api/auth/progress', {
                        method: 'POST', headers: {'Content-Type':'application/json'},
                        body: JSON.stringify({ 
                            username: sessionNick, protocol: 'v1', level: currentLevel, 
                            timeMs: totalTimeMs, completed: false, levelStartedAt: levelStartMs,
